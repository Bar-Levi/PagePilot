...
export function FaqSection({ headline, questions, onUpdate }: FaqSectionData & { onUpdate: (d: Partial<FaqSectionData>) => void }) {
  ...
  const handleQuestionUpdate = (index: number, field: 'question' | 'answer', value: string) => {
    const updatedQuestions = [...displayQuestions];
          ...
          {displayQuestions.map((faq, index) => (
            <AccordionItem key={index} value={`item-${index}`}>
              <AccordionTrigger 
                ...
                contentEditable
                suppressContentEditableWarning
                onBlur={(e) => handleQuestionUpdate(index, 'question', e.currentTarget.textContent || '')}
              >
                {faq.question}
              </AccordionTrigger>
              <AccordionContent 
                ...
                contentEditable
                suppressContentEditableWarning
                onBlur={(e) => handleQuestionUpdate(index, 'answer', e.currentTarget.textContent || '')}
              >
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
